...
from simulation_Pisa import SimulationBPSIM
import time
...
class alpha_configuration:
    ...
    # log pisa
    def __init__(self, path_log, path_bpmn, alpha_start, start_activities, parallel_activities, needEndActivities, already_timestamped): 
        self.log=xes_importer.apply(path_log) # real log
        self.path_bpmn=path_bpmn
        self.getTaskBPMN()
        self.alpha={'RADIOLOGIA ESECUZIONE TAC': 0.0,
                    'RADIOLOGIA REFERTAZIONE TAC': 0.0,
                    'RADIOLOGIA ESECUZIONE ECO': 0.0,
                    'RADIOLOGIA ESECUZIONE RX': 0.0,
                    'RADIOLOGIA REFERTAZIONE RX': 0.0,
                    'RADIOLOGIA ESECUZIONE Angio': 0.0,
                    'RADIOLOGIA REFERTAZIONE Angio': 0.0,
                    'RADIOLOGIA ESECUZIONE RMN': 0.0,
                    'RADIOLOGIA REFERTAZIONE RMN': 0.0,
                    'PRELIEVO': 0.0,
                    'DIMISSIONE': 0.0,
                    'PRESTAZIONIPS': 0.0,
                    'VISITA': 0.0}
    ...
    def addStartTimestampAlpha(self, alpha_log, alpha):
    ...
    def addStartTimestampAlpha_2(self, alpha_log, alpha): 
        ...
        for case_index, case in enumerate(alpha_log):
            for event_id,event in enumerate(case):
                if event['concept:name'] not in self.needEndActivities:
                    if event['concept:name'] in self.start_activities: # or event['concept:name'] not in self.alpha.keys():
                        event["start:timestamp"]=event['time:timestamp']
                    # if the event name is in already_timestamped it means that it has already the start:timestamp
                    elif event['concept:name'] in self.already_timestamped:
                        pass
                    else:
                        maximum=event["time:timestamp"]
                        idx = 1
    ...
    def computeRiemannDifference(self, real_distribution, simulated_distribution): 
        ...
        maximum, max_real, max_sim, mode_real, mode_sim= self.computeMaximum(real_distribution, simulated_distribution)
    ...
    def computeMaximum(self, real_distribution, simulated_distribution): 
        ...
        for i in range(1000):
            ...
            if simulated_distribution[0] == 'Constant':
                ...
                # UserDistribution
                if simulated_distribution[0] == 'UserDistribution':
                    SD = simulated_distribution[2].iloc[:,0]
        ...
        return maximum, max_real, max_sim, mode_real, mode_sim
    ...
    def sim_alpha(self, errore):     
        ...
        alpha_log=self.addStartTimestampAlpha_2(self.log, self.alpha)
        xes_exporter.apply(alpha_log, './alpha_log.xes')
        # call Class SimulationBPSim
        bpsim=SimulationBPSIM(alpha_log, self.path_bpmn)
